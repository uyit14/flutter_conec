...
class _ItemMyPostState extends State<ItemMyPost> {
  ...
  void requestPush(bool isPush, bool isPriority) {
    _postActionBloc.requestPushMyPost(widget.myPost.postId, isPush: isPush, isPriority: isPriority);
    _postActionBloc.pushMyPostStream.listen((event) {
      switch (event.status) {
        ...
        case Status.COMPLETED:
          ...
          Fluttertoast.showToast(msg: event.data, textColor: Colors.black87);
          break;
        case Status.ERROR:
          setState(() {
            _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return _isLoading
        ...
        : InkWell(
            onTap: () {
              if (widget.myPost.topicId ==
                  "333f691d-6595-443d-bae3-9a2681025b53") {
                Navigator.of(context).pushNamed(NewsDetailPage.ROUTE_NAME,
                    arguments: {'postId': widget.myPost.postId, 'owner': "00"});
              } else if (widget.myPost.topicId ==
                            ...
                            child: IconSlideAction(
                              ...
                              onTap: () {
                                Helper.showDeleteDialog(context, "Xoá tin",
                                  ...
                                  _postActionBloc.deleteMyPostStream
                                      .listen((event) {
                                    switch (event.status) {
                                      case Status.LOADING:
                                        break;
